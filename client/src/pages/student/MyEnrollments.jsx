            ...
            {enrolledCourses.map((course, index) => (
              <tr key={index} className='border-b border-gray-500/20 '>
                <td className='flex items-center gap-4 px-4 py-3'>
                  <img src={course.courseThumbnail} alt="" className='w-14
                   sm:w-24 md:w-28' />
                  <div className='flex-1'>
                    ...
                    <Line strokeWidth={2} percent={progressArray[index] ? 
                      ...
                      strokeColor="#3b82f6" trailWidth={2} trailColor="#e5e7eb" 
                      style={{ width: '100%' }
                ...
                <td className='px-4 py-3 max-sm:text-right'>
                  <button className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white ' onClick={()=>navigate('/play/'+course._id)}>
                    {progressArray[index] &&
